...
namespace BugDetectorGP.Scans.SourceCode.Anylazer
{
    public class Model
    {
        ...
        string pattern1 = @"SqlConnection.*\+";

        bool isContain(string method)
        {
            //    Console.WriteLine(method);
            return Regex.IsMatch(method, pattern1, RegexOptions.IgnoreCase)| Regex.IsMatch(method, pattern, RegexOptions.IgnoreCase)|Regex.IsMatch(method, Pattern, RegexOptions.IgnoreCase) | Regex.IsMatch(method, sqlPattern, RegexOptions.IgnoreCase) | Regex.IsMatch(method, sqlInjectionPattern, RegexOptions.IgnoreCase);
        }
